    ...
    <div ref={wrapRef} className="relative shrink-0">
      ...
      >
        ...
        {announcements.length > 0 && (
          <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-[#E31E24] ring-2 ring-[#0A0605]" aria-hidden />
        )}
      </button>

      {open && (
        <div className="absolute right-0 top-10 z-[70] w-72 overflow-hidden rounded-xl border border-white/10 bg-[#170D0C]/98 shadow-2xl backdrop-blur-md sm:w-80">
            ...
            {announcements.length === 0 ? (
              ...
              announcements.map((a) => (
                <div key={a.id} className="flex items-start gap-2.5 border-b border-white/5 px-3.5 py-3 last:border-0">
                  <Megaphone className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#FFC94A]" />
