          ...
          {continueItems.length === 0 ? (
            ...
            <div className="space-y-3">
              {continueItems.map((item) => (
                <div
                  key={item.show.id}
                  className="card-surface group flex items-center gap-4 overflow-hidden rounded-card p-3 transition hover:bg-[#141416]"
                >
                  <button
                    onClick={() =>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
              {watchlist.map((s) => (
                <div key={s.id} className="group relative">
                  ...
                  >
                    <div className="relative aspect-[2/3] overflow-hidden rounded-[10px] bg-[#141416] ring-1 ring-white/[0.07] shadow-card transition duration-300 group-hover:-translate-y-1 group-hover:ring-2 group-hover:ring-[#2050D8]/50">
                      <img
                        src={s.poster_url ?? ''}
                        alt={s.title}
                        loading="lazy"
